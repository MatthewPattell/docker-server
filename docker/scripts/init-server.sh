...
done < ${PROJECT_CONTAINER_DIR}/docker/ssh/authorized_keys

# Install deploy key and ssh config if exits
DEPLOY_KEY=${PROJECT_CONTAINER_DIR}/docker/ssh/site_deploy_key
SSH_CONFIG=${PROJECT_CONTAINER_DIR}/docker/ssh/config
...
cd ${PROJECT_CONTAINER_DIR} && composer install
